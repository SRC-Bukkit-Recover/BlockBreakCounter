...
public class Commands implements org.bukkit.command.CommandExecutor
{
  ...
  public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) { if (args.length > 0) {
      if (args[0].equalsIgnoreCase("help")) {
        ...
        if (sender.hasPermission("blockbreakcounter.add")) {
          if (args.length != 4) {
            ...
            try {
              value = Integer.valueOf(args[3]).intValue();
            } catch (IllegalArgumentException e) { int value;
      ...
      sender.sendMessage(ChatColor.BLUE + "-------------------------------------");
      sender.sendMessage(ChatColor.GOLD + "Plugin: " + ChatColor.WHITE + plugin.getDescription().getName());
      sender.sendMessage(ChatColor.GOLD + "Version: " + ChatColor.WHITE + plugin.getDescription().getVersion());
      sender.sendMessage(ChatColor.GOLD + "Authors: " + ChatColor.WHITE + plugin.getDescription().getAuthors().toString());
      sender.sendMessage(ChatColor.BLUE + "-------------------------------------");
    }
    return true;
  }
